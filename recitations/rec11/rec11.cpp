...
Node* reverse(const Node* hp) {
    Node* headNode =  new Node{hp->data};
    hp=hp->next;
    while(hp){
        listAddHead(headNode,hp->data);
        hp=hp->next;
    }
    return headNode;
...
const Node* isSublist(const Node* pattern, const Node* target){
    ...
    while(target){
        if(target->data==pattern->data){
            const Node* temp_targ = target;
            const Node* temp_pat = pattern;
            while(temp_pat && temp_targ){
                if (temp_pat -> next== nullptr){
                    if(temp_pat -> data == temp_targ->data){
                        return target;
                    }
                }
                temp_pat=temp_pat->next;
                temp_targ=temp_targ->next;
            }
        }
        target=target->next;
    }
...
const Node* sharedListUsingSet(const Node* listA, const Node* listB){
    unordered_set<const Node*> map;
    while(listA){
        map.insert(listA);
        listA=listA->next;
    }
    while(listB) {
        if(map.find(listB)!= map.end()){
            return listB;
        }
        listB=listB->next;
    }
...
const Node* optional(const Node* listA, const Node* listB){
